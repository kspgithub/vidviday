<div>
    ...
    @else
        ...
        <form wire:submit.prevent="saveItem()">
            <div class="row mb-3">
                <label class="col-md-2" for="landing_id">@lang('Place') <span class="text-danger">*</span></label>
                <div class="col-md-10">
                    <x-input.select2wire name="landing_id" id="landing_id" wire:model="landing_id"
                                         required>
                        <option value="0">Оберіть місце</option>
                        @foreach($options as $value=>$text)
                            <option
                                value="{{$value}}" {{$value === $landing_id ? 'selected' : ''}}>{{$text}}</option>
                        @endforeach
                    </x-input.select2wire>
