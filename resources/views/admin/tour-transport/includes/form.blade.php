<div>
    ...
    @else
        ...
        <form wire:submit.prevent="saveItem()">
            <div class="row mb-3">
                <label class="col-md-2" for="transport_id">@lang('Transport') <span class="text-danger">*</span></label>
                <div class="col-md-10">
                    <x-input.select2wire name="transport_id" id="transport_id" wire:model="transport_id"
                                         required>
                        <option value="0">Оберіть транспорт</option>
                        @foreach($options as $value=>$text)
                            <option
                                value="{{$value}}" {{$value === $transport_id ? 'selected' : ''}}>{{$text}}</option>
                        @endforeach
                    </x-input.select2wire>
