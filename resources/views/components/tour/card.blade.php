    ...
    <div class="thumb-content">
        ...
        @if($mode === 'thumb')
            ...
            <div class="thumb-info">
                <span class="thumb-info-time text">{{$tour->duration}}д {{$tour->nights > 0 && $tour->nights !== $tour->duration ? '/ '.$tour->nights.'н' : ''}}</span>
                ...
                <span class="thumb-info-people text">{{$schedule->places}}<span class="tooltip-wrap black">
                        <span class="tooltip text text-sm light">{{$schedule->places === 0 ? 'Немає місць' : 'Місця закінчуються'}}</span>
                    </span></span>
                @endif
            </div>
            <div class="thumb-price">
                ...
                <span class="discount">{{ceil($schedule->commission)}} грн. <span class="tooltip-wrap red"><span class="tooltip text text-sm light">Комісія агента</span></span></span>
                @endif
            </div>
        <a href="{{route('tour.order', $tour)}}" class="btn type-1 btn-block">Замовити Тур</a>
        @else
            <div class="text">
                <p>{{str_limit($tour->short_text, 150)}} <a href="{{$tour->url}}" class="btn btn-read-more text-bold">Більше</a></p>
            </div>
        @endif
    </div>
