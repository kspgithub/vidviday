<template>
    <div class="tab-top-part active" data-tab="1">
        <div class="or">
            <span class="text">{{ __('auth.or') }}</span>
        </div>
        <div class="spacer-sm"></div>

        <a v-bind="$buttons.auth.login_facebook" href="/auth/login/facebook" class="btn type-1 btn-block btn-fb">
            ...
            {{__('auth.sign-in-with', {provider: 'Facebook'})}}</a>

        <div class="spacer-xs"></div>

        <a v-bind="$buttons.auth.login_google" href="/auth/login/google" class="btn type-1 btn-block btn-google">
            ...
            {{ __('auth.sign-in-with', {provider: 'Google'}) }}</a>
    </div>
</template>
