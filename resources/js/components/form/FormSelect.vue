<template>
    ...
    >
        ...
        <div ref="optWrapperRef" class="optWrapper">
            <ul class="options">
                <li v-if="search">
                    <input ref="search" v-model="searchValue" type="text" @input="searchValue = $event.target.value" />
                </li>
                ...
                <li
                    v-if="multiple && selected.length === filteredOptions.length - 1"
                    class="opt"
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const selectedText = computed(() => {
            ...
            let options = props.options.filter(o =>
                props.multiple ? selectedValue.includes(toRaw(o).value) : toRaw(o).value == props.modelValue,
            )
        ...
        const filteredOptions = computed(() => {
            return props.options.filter(
                (o, i) => i === 0 || o.text.toLowerCase().indexOf(searchValue.value.toLowerCase()) > -1,
            )
        })
