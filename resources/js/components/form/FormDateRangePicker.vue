<template>
    <div class="datepicker-input vue-datepicker" ref="componentRef"
         :class="{invalid: errorFromMessage || errorToMessage, active: active}">
        <span class="datepicker-placeholder"
              @click="toggle()"
              :data-tooltip="errorTooltip"
        >{{ placeholder }}<span v-if="required && !dirty">*</span></span>
        <div class="datepicker-toggle">
            <div ref="pickerRef"></div>
        </div>
        <input type="hidden" :name="dateFromName" v-model="fromValue">
        <input type="hidden" :name="dateToName" v-model="toValue">
    </div>
</template>
