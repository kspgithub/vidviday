    ...
    <div class="contacts-block">
        ...
        <form action="/" @submit="submitForm" method="POST">
            ...
            <vue-recaptcha v-if="useRecaptcha" :sitekey="sitekey"
                           ...
                           ref="recaptcha"
            >
                <button id="b8" type="submit" class="btn type-2 btn-block" :disabled="request" @click="validateForm">
                    {{ __('forms.send-message') }}
                </button>
            </vue-recaptcha>
            <template v-if="!useRecaptcha">
                <button id="b9" type="submit" class="btn type-2 btn-block" :disabled="request" @click="validateForm">
                    {{ __('forms.send-message') }}
                </button>
            </template>
