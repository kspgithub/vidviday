        ...
        <div class="row mb-10">
            <div class="col-12">
                <span class="text-sm text-medium title">
                    {{
                        program_type === 1
                            ? __('order-section.details.departure-place-date')
                            : __('order-section.details.departure-date')
                    }}*
                </span>
            </div>
        ...
        <div class="row">
            <div class="col-12">
                <span class="text-sm text-medium title">
                    {{
                        program_type === 1
                            ? __('order-section.details.return-place-date')
                            : __('order-section.details.return-date')
                    }}*
                </span>
            </div>
            <div class="col-12 col-md-6">
...
export default {
    ...
    setup() {
        ...
        if (
            start_date.value &&
            end_date.value &&
            moment(end_date.value, 'DD.MM.YYYY').isBefore(moment(start_date.value, 'DD.MM.YYYY'))
        ) {
            end_date.value = moment(start_date.value, 'DD.MM.YYYY').add(1, 'day').format('DD.MM.YYYY')
        }

        const endMinDate = computed(() => {
            return start_date.value ? start_date.value : minDate.value
        })

        const startMaxDate = computed(() => {
            return end_date.value ? end_date.value : ''
        })
