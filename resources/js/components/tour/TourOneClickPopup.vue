<template>
    <popup size="size-1" :active="popupOpen" @hide="closePopup()">
        <div v-if="showForm" class="popup-align">
            ...
            <form :action="action" method="POST" class="row">
                ...
                <div class="col-md-6 col-12">
                    <form-input id="order-email" v-model="email" :label="__('forms.email')" name="email" />
                </div>
                ...
                <div class="col-12">
                    ...
                    <div class="text-center">
                        <button type="submit" :disabled="request" class="btn type-1" @click="submitForm">
                            {{ __('forms.order') }}
                        </button>
                    </div>
                </div>
    ...
    setup(props) {
        ...
        const { validate, errors } = useForm({
            validationSchema: {
                ...
                schedule_id: () => {
                    return props.schedules.find(s => s.id === data.value.schedule_id)
                        ? true
                        : __('validation.select-tour-date')
                },
            },
        })
        ...
        const submitForm = async () => {
            ...
            if (!result.valid) {
                ...
                request.value = true
                axios
                    .post(props.action, Object.assign({}, data.value))
                    .then(({ data }) => {
