...
export const useTourCard = tour => {
    ...
    const currencyPrice = computed(() => {
        return currentSchedule.value
            ? (currentSchedule.value.price / currencyRate.value).toFixed(0)
            : (tour.price / currencyRate.value).toFixed(0)
    })
    const currencyCommission = computed(() => {
        return currentSchedule.value
            ? (currentSchedule.value.commission / currencyRate.value).toFixed(0)
            : (tour.commission / currencyRate.value).toFixed(0)
    })
    ...
    const onlyQuick = computed(() => {
        return (
            ...
            (currentSchedule.value.places_available === 0 ||
                (currentSchedule.value.places_available >= 2 && currentSchedule.value.places_available <= 10))
        )
    })
