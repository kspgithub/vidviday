    ...
    <form action="/" class="calc-form vue-calc">
        ...
        <div class="calc">
            ...
            <div class="calc-footer">
                <span class="text-sm"
                    >{{ __('tours-section.total-sum') }}: <span class="calc-total-price">{{ total }}</span>
                    <sup>{{ __('common.currency.uah') }}</sup></span
                >
            </div>
        </div>
    </form>
...
export default {
    ...
    setup(props) {
        ...
        const total = computed(() => {
            let totalPrice = 0
            items.value
                .filter(it => selected.value.indexOf(it.id) !== -1)
                .forEach(it => {
