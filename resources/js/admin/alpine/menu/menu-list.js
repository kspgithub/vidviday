...
export default menu => ({
    ...
    onDrop(evt) {
        if (!this.childDrag && this.dragging !== null && this.dropping !== null) {
            if (this.dragging < this.dropping) {
                this.items = [
                    ...this.items.slice(0, this.dragging),
                    ...this.items.slice(this.dragging + 1, this.dropping + 1),
                    this.items[this.dragging],
                    ...this.items.slice(this.dropping + 1),
                ]
            } else {
                this.items = [
                    ...this.items.slice(0, this.dropping),
                    this.items[this.dragging],
                    ...this.items.slice(this.dropping, this.dragging),
                    ...this.items.slice(this.dragging + 1),
                ]
            }
    ...
    changeStatus(item) {
        axios
            .patch(`/admin/site-menu/${item.id}/status`)
            .then(({ data }) => {
                if (data.result === 'success') {
                    toast.success(data.message)
                }
