...
export default (params) => ({
    ...
    getValue(attribute) {
        return attribute.value || (attribute.options?.find(o => o.value === this.schedule[attribute.key]) || {text: this.schedule[attribute.key]}).text
    },
    initTourSelect() {
        setTimeout(() => {
            ...
            jQuery(tourSelectBox).on('select2:select', (e) => {
                ...
                let option = tourAttribute.options.find(o => o.value === this.schedule.tour_id) || {text:this.schedule.tour_id}
    ...
    loadSchedules() {
        if(this.schedule.tour_id) {
            axios.get(`/api/tours/${this.schedule.tour_id}/all-schedules`)
                .then((response) => {
                    ...
                    let option = scheduleAttribute.options.find(o => o.value === this.schedule.schedule_id) || (this.schedule.schedule_id === 0 ? options.shift() : {text:this.schedule.schedule_id})
    ...
    updateAttribute(id, value) {
        return axios.patch(`/admin/order/${this.order.id}`, this.schedule/*{[id]: value}*/)
            .then(({data: response}) => {
