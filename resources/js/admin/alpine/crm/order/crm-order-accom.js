...
export default params => ({
    ...
    accommodationsForm: {
        ...
        ...params.roomTypes.reduce((rt, res) => {
            res[rt.value] = params.order.accommodation ? params.order.accommodation[rt.value] || '' : ''
            return res
        }, {}),
    },
    isEditable(id) {
    ...
    updateAccomm(id, value) {
        return axios
            .patch(`/admin/order/${this.order.id}/accomm`, { id, value })
            .then(({ data: response }) => {
                this.order = response.model
                this.accommodationsForm.other = this.order.accommodation ? this.order.accommodation.other || '' : ''
                this.accommodationsForm.other_text = this.order.accommodation
                    ? this.order.accommodation.other_text || ''
                    : ''
                for (let rt of this.roomTypes) {
                    this.accommodationsForm[rt.value] = this.order.accommodation
                        ? this.order.accommodation[rt.value] || ''
                        : ''
                }
