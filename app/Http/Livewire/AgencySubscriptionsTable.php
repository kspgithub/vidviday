...
class AgencySubscriptionsTable extends DataTableComponent
{
    ...
    {
        $item = AgencySubscription::query()->find($id);
        if ($item) {
            $item->status = AgencySubscription::STATUS_ACTIVE;
            $item->save();
        }
    }
    ...
    public function blockItem($id)
    {
        $item = AgencySubscription::query()->find($id);
        if ($item) {
            $item->status = AgencySubscription::STATUS_INACTIVE;
            $item->save();
        }
    }
    ...
    {
        $model = AgencySubscription::query()->find($this->parent_id);
        if ($model) {
            $this->validate();
            $item = new AgencySubscription();
            $item->name = $this->name;
